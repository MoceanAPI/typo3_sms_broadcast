...
namespace Mocean\MoceanApiBroadcast\Domain\Finishers;

use Mocean\MoceanApiBroadcast\Mocean\Sms;
use Mocean\MoceanApiBroadcast\Domain\Model\History;
...
class SmsFormFinisher extends AbstractFinisher
{
    /**
    ...
    /**
     * @param \Mocean\MoceanApiBroadcast\Domain\Repository\UserRepository
     */
    ...
    protected function executeInternal()
    {
        ...
        if($formValues['recipient'] == 1)
        {
            $this->sendAll($formValues['message']);
        }
        else if($formValues['recipient'] == 2)
        {
            $this->sendUsers($formValues['message'], $formValues['specificUsers']);
        }
        else if($formValues['recipient'] == 3)
        {
            $this->sendGroup($formValues['message'], $formValues['specificGroup']);
        }
        else if($formValues['recipient'] == 4)
        {
            $this->sendPhone($formValues['message'], $formValues['specificPhone']);
        }
    }
    
    /**
     * Send SMS to all FE users
     */
    public function sendAll($message)
    {
        ...
        foreach($users as $user)
        {
            ...
            if($userCountry)
            {
                $country = $this->getCountryCode($userCountry);
                if(!$country)
                    $country = GeneralUtility::makeInstance(ExtensionConfiguration::class)->get('mocean_api_broadcast', 'moceanCountry');
            }
            else
                $country = GeneralUtility::makeInstance(ExtensionConfiguration::class)->get('mocean_api_broadcast', 'moceanCountry');
            ...
            if($country)
            {
                ...
                $validRecipient = $request->checkCountryCode($recipient);
                if($validRecipient)
                {
                    $recipient = $validRecipient;
                    $request->setRecipient($validRecipient);
                }
                else
                {
                    $response = 'Invalid phone number for selected country';
                    $status = 'Fail';
                    $log = 'N/A';
                    $this->newHistory($recipient, $message, $response, $status, $log);
                    continue;
                }
            }
        ...
        if($total == $success)
            $this->successMessage($total);
        else
            $this->errorMessage($success, $total);
    }
    
    /**
     * Send SMS to selected FE users
     */
    public function sendUsers($message, $specificUsers)
    {
        if(!$specificUsers)
        {
            $message = GeneralUtility::makeInstance(FlashMessage::class,
                'Please select user(s) as recipient.',
                'No User(s) Selected',
    ...
    /**
     * Send SMS to selected FE usergroup
     */
    public function sendGroup($message, $specificGroup)
    {
        if($specificGroup == 0)
        {
            $message = GeneralUtility::makeInstance(FlashMessage::class,
                'Please select a user group as recipient.',
                'No Group Selected',
                FlashMessage::WARNING
            );
        ...
        $objectManager = GeneralUtility::makeInstance(ObjectManager::class); 
		$this->userRepository = $objectManager->get(UserRepository::class);
        $users = $this->userRepository->findByUserGroup($specificGroup);
        foreach($users as $user)
        {
            $recipient = $user->getTelephone();
    ...
    /**
     * Send SMS to entered phone numbers
     */
    public function sendPhone($message, $specificPhone)
    {
        if(!$specificPhone)
        {
            $message = GeneralUtility::makeInstance(FlashMessage::class,
                'Please enter phone number(s) as recipient.',
        ...
        $recipients = preg_split('~[^0-9]~i', $specificPhone, -1, PREG_SPLIT_NO_EMPTY);
    ...
    public function successMessage($total)
    {
        $message = GeneralUtility::makeInstance(FlashMessage::class,
            $total.' out of '.$total.' SMS sent.',
            'Success'
        );
    ...
    public function errorMessage($success, $total)
    {
        $message = GeneralUtility::makeInstance(FlashMessage::class,
            $success.' out of '.$total.' SMS sent, check SMS Transaction History for more details.',
            'Error',
            FlashMessage::ERROR
        );
    ...
    /**
     * Send SMS request and process response
     */
    public function sendSms($request) 
    {
        ...
        $request->setSender(GeneralUtility::makeInstance(ExtensionConfiguration::class)->get('mocean_api_broadcast', 'moceanMessageFrom'));
        $jsonSms = $request->sendBroadcast(GeneralUtility::makeInstance(ExtensionConfiguration::class)->get('mocean_api_broadcast', 'moceanMessageFrom'));

        if (count($jsonSms) == count($jsonSms, COUNT_RECURSIVE)) 
        {
            $errMsg = $jsonSms['err_msg'];
            $status = $jsonSms['status'];
        }
        else 
        {
            $errMsg = $jsonSms['messages'][0]['err_msg'];
            $status = $jsonSms['messages'][0]['status'];
        }
        ...
        $this->newHistory($request->getRecipient(), $request->getMessage(), $response, $status, $jsonSms['log']);
        
        return $noError;
    }
    
    /**
     * Insert new sms transaction history
     */
    public function newHistory($recipient, $message, $response, $status, $log)
    {
        $objectManager = GeneralUtility::makeInstance(ObjectManager::class); 
		$this->historyRepository = $objectManager->get(HistoryRepository::class);
        
        $history = GeneralUtility::makeInstance(History::class);
        $history->setDatetime(time());
        $history->setRecipient($recipient);
        $history->setMessage($message);
        $history->setResponse($response);
        $history->setStatus($status);
        $history->setSmsLog($log);
        $this->historyRepository->add($history);
        $objectManager->get(PersistenceManagerInterface::class)->persistAll();
    }
    
    /**
     * Check country code for users with country in personal information
     */
    public function getCountryCode($val)
    {
        $countries = array();
